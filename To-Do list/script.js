...
function loadData() {
  const saved = localStorage.getItem("userflowTasks");
  if (saved) tasks = JSON.parse(saved);
    ...
    tasks = [
      ...
      {
        id: 2,
        title: "Itendify the implementation team",
        status: "progress",
        priority: "normal",
        completed: false,
      },
      {
        id: 3,
        title: "Batch schedule dowload/process",
        status: "pending",
        priority: "critical",
        completed: false,
      },
...
function saveData() {
  localStorage.setItem("userflowTasks", JSON.stringify(tasks));
}
...
function renderTasks() {
  ...
  document.getElementById("onHoldTasks").innerHTML = onHold.length
    ? onHold
          ...
          (t) =>
            `<div class="task-item">
                            ...
                            <div class="task-content">
                                <div class="task-tittle ${t.completed ? "completed" : ""}">${t.title}</div>
                            </div>
        ...
        .join("")
    : '<p style="color: #9cq3af; padding:20px;">No tasks on Hold</p>';

    document.getElementById("completedTasks").innerHTML = completed.length ? completed.map(
        (t) => 
            `<div class="task-item">
                            <div class="task-checkbox completed" onclick="toggleTask(${t.id})"></div>
                            <div class="task-content">
                                <div class="task-tittle completed">${t.title}</div>
                            </div>
